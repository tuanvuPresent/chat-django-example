...
INSTANCE_GEN_ID = 1
EPOCH_TIME = int(datetime.datetime(
    year=2022, month=9, day=1).timestamp()) * 1000
...
class UuidGenSingletonGroup:
    ...
    def __new__(cls, name):
        if not cls._instances.get((cls.__name__, name)):
            cls._instances[(cls.__name__, name)] = super(
                UuidGenSingletonGroup, cls).__new__(cls)
            cls.uuid = UuidGenerator(
                epoch=EPOCH_TIME, instance=INSTANCE_GEN_ID)
